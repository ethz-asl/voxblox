...
EsdfOccFiestaIntegrator::EsdfOccFiestaIntegrator(const Config& config,
...
void EsdfOccFiestaIntegrator::updateFromOccBlocks(const BlockIndexList& occ_blocks,
                                                  bool incremental) {
      ...
      if(occupancy_voxel.observed)
      {
        ...
        if(esdf_voxel.self_idx(0) == UNDEF) //not yet assigned
        {
          ...
          esdf_voxel.distance = esdf_voxel.behind ? 
              -config_.behind_surface_m : config_.default_distance_m; 
        }
      }
    }
  }
...
void EsdfOccFiestaIntegrator::deleteFromList(EsdfVoxel* occ_vox, 
  ...
  if (cur_vox->prev_idx(0) != UNDEF)
  {
    ...
    prev_vox->next_idx = cur_vox->next_idx; // a <-> b <-> c , delete b, a <-> c
  }
  else
...
// Reference: Han. L, et al., Fast Incremental Euclidean Distance Fields for Online
// Motion Planning of Aerial Robots, IROS 2019
// A mapping system called Fiesta is proposed to build global ESDF map incrementally. 
// By introducing two independent updating queues for inserting and deleting obstacles 
// separately, and using Indexing Data Structures and Doubly Linked Lists for map 
// maintenance, our algorithm updates as few as possible nodes using a BFS framework. 
// The ESDF mapping has high computational performance and produces near-optimal results.
// Code: https://github.com/HKUST-Aerial-Robotics/FIESTA
...
void EsdfOccFiestaIntegrator::updateESDF(){
  ...
  while (!insert_list_.empty()) {
    ...
    {
      EsdfVoxel* coc_vox = esdf_layer_->getVoxelPtrByGlobalIndex(cur_vox->coc_idx);
  ...
  while (!delete_list_.empty()) {
    ...
    for (GlobalIndex temp_vox_idx = cur_vox->head_idx; temp_vox_idx(0) != UNDEF; 
          ...
          if (voxInRange(nbr_vox_idx))
          {
            ...
            if (nbr_vox->observed && nbr_coc_vox_idx(0) != UNDEF)
            {
              ...
              if(nbr_coc_occ_vox->occupied) 
              {
                ...
                {
                  temp_vox->distance = temp_vox->behind ? -temp_dist: temp_dist;
      ...
      if (temp_vox->coc_idx(0) != UNDEF) 
      {
        ...
        EsdfVoxel* temp_coc_vox = esdf_layer_->getVoxelPtrByGlobalIndex(temp_vox->coc_idx);
...
void EsdfOccFiestaIntegrator::loadInsertList(const GlobalIndexList& insert_list)
{
  insert_list_ = insert_list;
}

void EsdfOccFiestaIntegrator::loadDeleteList(const GlobalIndexList& delete_list)
{
