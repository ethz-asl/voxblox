...
EsdfTangoIntegrator::EsdfTangoIntegrator(const Config& config,
                               Layer<TangoTsdfVoxel>* tsdf_layer,
...
void EsdfTangoIntegrator::getSphereAroundPoint(
...
void EsdfTangoIntegrator::addNewRobotPosition(const Point& position) {
...
void EsdfTangoIntegrator::updateFromTsdfLayerBatch() {
...
void EsdfTangoIntegrator::updateFromTsdfLayerBatchOccupancy() {
  esdf_layer_->removeAllBlocks();
  BlockIndexList tsdf_blocks;
  tsdf_layer_->getAllAllocatedBlocks(&tsdf_blocks);
  updateFromTsdfBlocksAsOccupancy(tsdf_blocks);
}
void EsdfTangoIntegrator::updateFromTsdfLayerBatchFullEuclidean() {
  esdf_layer_->removeAllBlocks();
  BlockIndexList tsdf_blocks;
  tsdf_layer_->getAllAllocatedBlocks(&tsdf_blocks);
...
void EsdfTangoIntegrator::updateFromTsdfLayer(bool clear_updated_flag) {
  BlockIndexList tsdf_blocks;
...
void EsdfTangoIntegrator::updateFromTsdfBlocks(const BlockIndexList& tsdf_blocks) {
  constexpr bool push_neighbors = true;
  updateFromTsdfBlocks(tsdf_blocks, push_neighbors);
}

void EsdfTangoIntegrator::updateFromTsdfBlocksFullEuclidean(
  ...
  for (const BlockIndex& block_index : tsdf_blocks) {
    ...
    const Block<TangoTsdfVoxel>& tsdf_block =
    ...
    for (size_t lin_index = 0u; lin_index < num_voxels_per_block; ++lin_index) {
      const TangoTsdfVoxel& tsdf_voxel = tsdf_block.getVoxelByLinearIndex(lin_index);
...
void EsdfTangoIntegrator::updateFromTsdfBlocksAsOccupancy(
    const BlockIndexList& tsdf_blocks) {
  DCHECK_EQ(tsdf_layer_->voxels_per_side(), esdf_layer_->voxels_per_side());
  timing::Timer esdf_timer("esdf");
...
void EsdfTangoIntegrator::pushNeighborsToOpen(const BlockIndex& block_index,
...
void EsdfTangoIntegrator::processRaiseSet() {
...
void EsdfTangoIntegrator::processOpenSet() {
  size_t num_updates = 0u;
  while (!open_.empty()) {
...
void EsdfTangoIntegrator::processOpenSetFullEuclidean() {
  size_t num_updates = 0u;
  while (!open_.empty()) {
    VoxelKey kv = open_.front();
    open_.pop();
...
void EsdfTangoIntegrator::getNeighborsAndDistances(
...
void EsdfTangoIntegrator::getNeighbor(const BlockIndex& block_index,
                                 const VoxelIndex& voxel_index,
