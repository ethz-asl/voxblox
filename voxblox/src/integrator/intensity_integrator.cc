...
void IntensityIntegrator::addIntensityBearingVectors(
  ...
  const int intensity_prop_voxel_radius = 2;
  ...
  for (size_t i = 0; i < bearing_vectors.size(); ++i) {
    ...
    for (int voxel_offset = -intensity_prop_voxel_radius;
         voxel_offset <= intensity_prop_voxel_radius; voxel_offset++) {
      close_voxel =
