 ...
 * TODO(mereweth@jpl.nasa.gov) - specialize this function for Layer<TsdfVoxel>
 * with something that makes more sense for projective distance.
 */
template <>
inline void Layer<TsdfVoxel>::fillSphereAroundPoint(const Point& center,
     ...
     for (FloatingPoint z = -radius; z <= radius; z += voxel_size_) {
       ...
       if (point.squaredNorm() <= radius_squared_norm) {
         ...
         const Block<TsdfVoxel>::Ptr block_ptr =
             this->allocateBlockPtrByCoordinates(point);
 ...
 */
template <>
inline void Layer<TsdfVoxel>::clearSphereAroundPoint(const Point& center,
                                              const FloatingPoint radius,
                                              const FloatingPoint weight) {
  // search a cube with side length 2*radius
  for (FloatingPoint x = -radius; x <= radius; x += voxel_size_) {
