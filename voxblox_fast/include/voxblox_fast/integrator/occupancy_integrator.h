#ifndef VOXBLOX_FAST_INTEGRATOR_OCCUPANCY_INTEGRATOR_H_
#define VOXBLOX_FAST_INTEGRATOR_OCCUPANCY_INTEGRATOR_H_
  ...
  void integratePointCloud(const Transformation& T_G_C,
    ...
    for (size_t pt_idx = 0; pt_idx < points_C.size(); ++pt_idx) {
      ...
      if (ray_distance < config_.min_ray_length_m) {
        ...
        if (occupied_cells.find(getGridIndexFromPoint(end_scaled)) ==
            occupied_cells.end()) {
          castRay(start_scaled, end_scaled, &global_voxel_indices);
    ...
    bool occupied = false;
    for (const AnyIndex& global_voxel_idx : free_cells) {
...
#endif  // VOXBLOX_FAST_INTEGRATOR_OCCUPANCY_INTEGRATOR_H_
